...
jQuery(function( $ ) {
    ...
    // обнуляем поля формы при скрытии модального окна
    // $('#myModalPortrait').on('hide.bs.modal', function (e) {
     //$('#myModalPortrait #inputPhone').val('');
     //$('#myModalPortrait #inputEmail').val('').closest('div.form-group').removeClass('has-error').removeClass('has-success');
     //removeError();
     //});
    ...
    $('.send-btn').on('click', function(){
        ...
        if( $($modalID + ' #inputPhone').val() == '' && $($modalID + ' #inputEmail').val() == '' ) {
            $($modalID + ' #error').text('Не заполнено ни одно из полей формы');
            $($modalID + ' .alert').fadeIn(250);
        } else {
            var pattern = /^([a-z0-9_\.-])+@[a-z0-9-]+\.([a-z]{2,4}\.)?[a-z]{2,7}$/i;

            if( ( $($modalID + ' #inputPhone').val() == '' && pattern.test($($modalID + ' #inputEmail').val()) ) ||
                ( $($modalID + ' #inputPhone').val() != '' && $($modalID + ' #inputEmail').val() == '' ) ||
                ( $($modalID + ' #inputPhone').val() != '' && $($modalID + ' #inputEmail').val() != '' && pattern.test($($modalID + ' #inputEmail:visible').val()) )
            ) {
